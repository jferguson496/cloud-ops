data "aws_ami" "eks_worker_blue" {
  filter {
    name   = "name"
    values = [var.eks_worker_blue_ami_version == "latest" ? "amazon-eks-node-${var.eks_kubernetes_version_blue}-*" : "amazon-eks-node-${var.eks_kubernetes_version_blue}-${var.eks_worker_blue_ami_version}"]
  }
...
resource "aws_launch_template" "worker_blue" {
  name                   = "${local.stack_name}_worker-blue"
  image_id               = data.aws_ami.eks_worker_blue.id
  ...
  tag_specifications {
    ...
    tags = merge({
      "Name"                                      = "${local.stack_name}_eks-worker-blue"
...
resource "aws_autoscaling_group" "worker_blue" {
  name                = "${local.stack_name}_eks-worker-blue"
  min_size            = var.eks_worker_blue_asg_min_size
  max_size            = var.eks_worker_blue_asg_max_size
  desired_capacity    = var.eks_worker_blue_asg_min_size
  ...
  launch_template {
    id      = aws_launch_template.worker_blue.id
    version = "$Latest"
  }
  ...
  tags = [
    {
      key                 = "Name"
      value               = "${local.stack_name}_eks-worker-blue"
